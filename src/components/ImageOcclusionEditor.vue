...
import {
  useImageOcclusionEditor,
  type IoTool,
} from "../composables/useImageOcclusionEditor";
...
watch(() => props.occlusionMode, (val) => { if (val && val !== editor.occlusionMode.value) editor.setOcclusionMode(val); });
watch(() => editor.occlusionMode.value, (val) => emit("update:occlusionMode", val));
watch(() => props.modelValue, (val) => { if (JSON.stringify(val) !== JSON.stringify(editor.shapes.value)) editor.setShapes(val); });
watch(() => editor.shapes.value, (val) => emit("update:modelValue", val), { deep: true });
...
function findPolyVertex(pt: { x: number; y: number }): { shapeId: string; pointIndex: number } | null {
  ...
  if (editor.activeTool.value === "select") {
    ...
    if (handleEl && editor.selectedShape.value) {
      ...
      const anchors: Record<HandleDir, { x: number; y: number }> = {
        nw: { x: shape.x + shape.width, y: shape.y + shape.height }, n: { x: shape.x, y: shape.y + shape.height },
        ne: { x: shape.x, y: shape.y + shape.height }, e: { x: shape.x, y: shape.y },
        se: { x: shape.x, y: shape.y }, s: { x: shape.x, y: shape.y },
        sw: { x: shape.x + shape.width, y: shape.y }, w: { x: shape.x + shape.width, y: shape.y },
      };
...
function onPointerMove(e: PointerEvent) {
  ...
  if (editor.isDrawing.value) {
    ...
    let newX = anchorX, newY = anchorY, newW = 10, newH = 10;
    if (dir.includes("w") || dir.includes("e")) { newX = Math.min(dir.includes("w") ? pt.x : anchorX, dir.includes("w") ? anchorX : pt.x); newW = Math.max(10, Math.abs((dir.includes("w") ? anchorX : pt.x) - (dir.includes("w") ? pt.x : anchorX))); }
    if (dir.includes("n") || dir.includes("s")) { newY = Math.min(dir.includes("n") ? pt.y : anchorY, dir.includes("n") ? anchorY : pt.y); newH = Math.max(10, Math.abs((dir.includes("n") ? anchorY : pt.y) - (dir.includes("n") ? pt.y : anchorY))); }
    if (dir === "n" || dir === "s") { newX = shape?.x ?? newX; newW = shape?.width ?? newW; }
    if (dir === "e" || dir === "w") { newY = shape?.y ?? newY; newH = shape?.height ?? newH; }
    ...
    for (const orig of dragState.origins) {
      editor.resizeShape(orig.id, { x: orig.x + dx, y: orig.y + dy, width: orig.w, height: orig.h });
    }
  }
}
...
const selectedIsGrouped = computed(() => {
  ...
  if (ids.size < 2) {
    ...
    return editor.shapes.value.filter((s) => s.ordinal === editor.selectedShape.value!.ordinal).length > 1;
  }
...
const HANDLE_SCALE = computed(() => Math.max(imageNaturalWidth.value, imageNaturalHeight.value) / 800);
...
function getHandles(shape: OcclusionShape) {
  ...
  return [
    { dir: "nw" as HandleDir, cx: x, cy: y }, { dir: "n" as HandleDir, cx: x + w / 2, cy: y },
    { dir: "ne" as HandleDir, cx: x + w, cy: y }, { dir: "e" as HandleDir, cx: x + w, cy: y + h / 2 },
    { dir: "se" as HandleDir, cx: x + w, cy: y + h }, { dir: "s" as HandleDir, cx: x + w / 2, cy: y + h },
    { dir: "sw" as HandleDir, cx: x, cy: y + h }, { dir: "w" as HandleDir, cx: x, cy: y + h / 2 },
  ];
}
  ...
  <div class="flex flex-col gap-2">
    ...
    <div v-if="!readonly" class="flex items-center gap-1 flex-wrap">
      <button
        ...
        :class="['btn-pill !px-2', editor.activeTool.value === tool.key ? 'bg-accent text-accent-fg' : '']"
      ...
      >
        <svg width="18" height="18" viewBox="0 0 18 18" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
          ...
          <template v-if="tool.key === 'select'">
            <path d="M4 2 L4 13 L7.5 9.5 L10.5 15 L12.5 14 L9.5 8 L14 8 Z" fill="currentColor" stroke="none" />
          </template>
          <!-- Rectangle -->
      ...
      <button
        :class="['btn-pill', editor.occlusionMode.value === 'hide-all-guess-one' ? 'bg-accent text-accent-fg' : '']"
        @click="editor.setOcclusionMode('hide-all-guess-one')"
      >Hide All</button>
      <button
        :class="['btn-pill', editor.occlusionMode.value === 'hide-one' ? 'bg-accent text-accent-fg' : '']"
      ...
      >Group</button>
      <button
        v-if="selectedIsGrouped"
        class="btn-pill"
        @click="ungroupSelected"
      >Ungroup</button>
      <button
        class="btn-pill" style="color: var(--c-again)"
        :disabled="!editor.selectedShapeId.value"
      ...
      <div v-if="editingLabel" class="flex items-center gap-1 ml-2">
        ...
        >
        <button class="btn-pill" style="background: var(--c-accent); color: var(--c-accent-fg)" @click="saveLabel">OK</button>
      </div>
    </div>
      ...
      >
        ...
        <template v-for="shape in editor.shapes.value" :key="shape.id">
          <rect v-if="shape.type === 'rect'"
            :data-shape-id="shape.id" :x="shape.x" :y="shape.y" :width="shape.width" :height="shape.height"
            fill="#ffeba2" fill-opacity="0.6" :stroke="editor.selectedShapeIds.value.has(shape.id) ? '#2196f3' : '#2d2d2d'"
            :stroke-width="editor.selectedShapeIds.value.has(shape.id) ? STROKE_WIDTH : Math.max(1, STROKE_WIDTH / 2)" style="cursor: pointer"
          />
          <ellipse v-else-if="shape.type === 'ellipse'"
            :data-shape-id="shape.id" :cx="shape.x + shape.width / 2" :cy="shape.y + shape.height / 2"
            :rx="shape.width / 2" :ry="shape.height / 2"
            fill="#ffeba2" fill-opacity="0.6" :stroke="editor.selectedShapeIds.value.has(shape.id) ? '#2196f3' : '#2d2d2d'"
            :stroke-width="editor.selectedShapeIds.value.has(shape.id) ? STROKE_WIDTH : Math.max(1, STROKE_WIDTH / 2)" style="cursor: pointer"
          />
          <polygon v-else-if="shape.type === 'polygon' && shape.points"
          ...
          <template v-if="shape.type === 'text'">
            <rect
              :data-shape-id="shape.id" :x="shape.x" :y="shape.y" :width="Math.max(shape.width, 40)" :height="Math.max(shape.height, 20)"
              fill="transparent" :stroke="editor.selectedShapeIds.value.has(shape.id) ? '#2196f3' : 'transparent'"
              :stroke-width="editor.selectedShapeIds.value.has(shape.id) ? STROKE_WIDTH : 0" :stroke-dasharray="`${DASH_SIZE} ${DASH_SIZE / 2}`" style="cursor: pointer"
            />
            <text
              :x="shape.x + 4" :y="shape.y + Math.max(shape.height, 20) * 0.7"
              :font-size="Math.max(shape.height * 0.7, 12)" fill="#222" pointer-events="none"
              font-weight="bold"
              stroke="white" :stroke-width="Math.max(shape.height * 0.7, 12) * 0.15" paint-order="stroke"
            >{{ shape.label ?? 'Text' }}</text>
          </template>

          <!-- Ordinal + label on mask shapes -->
          <text v-if="shape.type !== 'text'" :x="shape.x + shape.width / 2" :y="shape.y + shape.height / 2"
            text-anchor="middle" dominant-baseline="central" font-size="14" font-weight="bold" fill="#333" pointer-events="none"
          >{{ shape.label ?? shape.ordinal }}</text>
        </template>

        <!-- Selection outlines for all selected shapes -->
        <template v-for="shape in editor.shapes.value.filter(s => editor.selectedShapeIds.value.has(s.id))" :key="'sel-' + shape.id">
          <rect v-if="!readonly"
            :x="shape.x - 2" :y="shape.y - 2"
            :width="shape.width + 4" :height="shape.height + 4"
            fill="none" stroke="#2196f3" :stroke-width="STROKE_WIDTH" :stroke-dasharray="`${DASH_SIZE} ${DASH_SIZE / 2}`" pointer-events="none"
          />
        </template>
        ...
        <template v-if="editor.selectedShape.value && !editor.multiSelected.value && !readonly">
          <template v-for="h in getHandles(editor.selectedShape.value)" :key="h.dir">
            <rect :data-handle="h.dir" :x="h.cx - HANDLE_TOUCH / 2" :y="h.cy - HANDLE_TOUCH / 2"
              :width="HANDLE_TOUCH" :height="HANDLE_TOUCH" fill="transparent" :style="{ cursor: cursors[h.dir] }"
            />
            <rect :x="h.cx - HANDLE_SIZE / 2" :y="h.cy - HANDLE_SIZE / 2" :width="HANDLE_SIZE" :height="HANDLE_SIZE"
              fill="white" stroke="#2196f3" :stroke-width="STROKE_WIDTH" :rx="STROKE_WIDTH" pointer-events="none"
            />
          </template>
        </template>

        <!-- Polygon vertex handles for selected polygon -->
        <template v-if="editor.selectedShape.value?.type === 'polygon' && editor.selectedShape.value.points && !editor.multiSelected.value && !readonly">
          <circle v-for="(pt, i) in editor.selectedShape.value.points" :key="'pv-' + i"
            :cx="pt.x" :cy="pt.y" :r="HANDLE_SIZE / 2"
            fill="white" stroke="#2196f3" :stroke-width="STROKE_WIDTH"
            style="cursor: move"
            :data-poly-vertex="i"
          />
        </template>
        ...
        <template v-if="editor.isDrawingPolygon.value && editor.polygonPoints.value.length > 0">
          <polyline
            ...
            fill="rgba(255,235,162,0.3)" stroke="#2196f3" :stroke-width="STROKE_WIDTH"
            :stroke-dasharray="`${DASH_SIZE} ${DASH_SIZE / 2}`" pointer-events="none"
          />
          <circle v-for="(pt, i) in editor.polygonPoints.value" :key="i"
            :cx="pt.x" :cy="pt.y" :r="HANDLE_SIZE / 2"
            :fill="i === 0 && editor.polygonPoints.value.length >= 3 ? '#4caf50' : 'white'"
            stroke="#2196f3" :stroke-width="STROKE_WIDTH / 2" pointer-events="none"
          />
        </template>
        ...
        <rect v-if="marquee"
          ...
          fill="rgba(33,150,243,0.1)" stroke="#2196f3" :stroke-width="STROKE_WIDTH / 2"
          :stroke-dasharray="`${DASH_SIZE / 2} ${DASH_SIZE / 4}`" pointer-events="none"
        />

        <!-- Inline text editor overlay -->
        <template v-if="inlineEditId">
          <!-- Click-away backdrop (covers the whole SVG) -->
          <rect x="0" y="0" :width="imageNaturalWidth" :height="imageNaturalHeight"
            fill="transparent" @pointerdown.stop="saveInlineEdit"
          />
          <foreignObject
            :x="editor.shapes.value.find(s => s.id === inlineEditId)?.x ?? 0"
            :y="(editor.shapes.value.find(s => s.id === inlineEditId)?.y ?? 0) - STROKE_WIDTH"
            :width="Math.max((editor.shapes.value.find(s => s.id === inlineEditId)?.width ?? 100) * 1.5, 200 * HANDLE_SCALE)"
            :height="Math.max((editor.shapes.value.find(s => s.id === inlineEditId)?.height ?? 30) + STROKE_WIDTH * 4, 40 * HANDLE_SCALE)"
          >
            <input
              ...
              :style="{
                ...
                border: `${STROKE_WIDTH}px solid #2196f3`, borderRadius: `${STROKE_WIDTH}px`,
